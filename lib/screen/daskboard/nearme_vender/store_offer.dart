...
class StoreOfferScreen extends StatefulWidget {
  ...
  const StoreOfferScreen({
    Key? key,
    this.initialIndex = 0,
    required this.storeId, // default first offer
  }) : super(key: key);
...
class _StoreOfferScreenState extends State<StoreOfferScreen> {
  ...
  controller.getStoreOfferApi(storeId: widget.storeId);
});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(() {
        ...
        if (controller.storeOfferList.isEmpty) {
        ...
        return Stack(
          children: [
            PageView.builder(
              ...
              itemCount: controller.storeOfferList.length,
              onPageChanged: (index) {
                setState(() => currentIndex = index);
              },
              itemBuilder: (context, index) {
                return _offerCard(controller.storeOfferList[index]);
              },
            ),
          ],
        );
